...
router.get('/:name',async (req,res)=>{
    try{
        ...
        if(voucher.length > 0) {
            res.status(200).json({voucher:voucher[0]});
        }
        else{
            res.status(404).json({message: "Voucher not found",voucher:[]});
        }
...
router.post('/',async (req,res)=>{
    try{
        ...
        if(voucher.length > 0) {
            ...
            const sql = `INSERT INTO VOUCHER (NAME, DISCOUNT,validity,CAP) VALUES (:name, :discount,TO_CHAR(:validity,'DD-MM-YYYY'),:CAP)`;
            const binds = {
                name: req.body.NAME,
